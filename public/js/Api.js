const API_DELAY = Math.floor(Math.random() * (4000 - 1000 + 1) + 1000);

export function getSubscribersData(query) {
  // 'http://billrun/api/aggregate?collection=lines&pipelines=[{%22$match%22:{%22charging_type%22:%22prepaid%22}}]';
  var promise = new Promise((resolve, reject) => {
    fetch(query).then(response => {
      if(response.ok) {
        response.json().then((body) => {
          if(body.status) {
            resolve(body.details);
          } else {
            reject(body);
          }
        }).catch(error => {
...
export function getMultipleData(request) {
