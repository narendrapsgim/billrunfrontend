...
export default class AdvancedFilter extends Component {
  ...
  renderTitles = (field, index, fields) => {
    const label = changeCase.titleCase(field.title || field.placeholder || field.id);
    const style = { width: `${100 / (fields.length === 0 ? 1 : fields.length)}%` };
    return (<th style={style} key={index}>{label}</th>);
  };
