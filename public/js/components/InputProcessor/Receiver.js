...
class Receiver extends Component {
      ...
      <form className="form-horizontal">
        ...
        <div className="form-group">
          ...
          <div className="col-xs-4">
            <input className="form-control" id="name" onChange={onSetReceiverField} value={connection.get('name', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="host" className="col-xs-2 control-label">Host</label>
          <div className="col-xs-4">
            <input className="form-control" id="host" onChange={onSetReceiverField} value={connection.get('host', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="user" className="col-xs-2 control-label">User</label>
          <div className="col-xs-4">
            <input className="form-control" id="user" onChange={onSetReceiverField} value={connection.get('user', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="password" className="col-xs-2 control-label">Password</label>
          <div className="col-xs-4">
            <input type="password" className="form-control" id="password" onChange={onSetReceiverField} value={connection.get('password', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="remote_directory" className="col-xs-2 control-label">Directory</label>
          <div className="col-xs-4">
            <input className="form-control" id="remote_directory" onChange={onSetReceiverField} value={connection.get('remote_directory', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="filename_regex" className="col-xs-2 control-label">Regex</label>
          <div className="col-xs-4">
            <input className="form-control" id="filename_regex" onChange={onSetReceiverField} value={connection.get('filename_regex', '')} />
          </div>
        </div>
        <div className="form-group">
          <label htmlFor="period" className="col-xs-2 control-label">Period</label>
          <div className="col-xs-4">
            <select className="form-control" id="period" onChange={onSetReceiverField} value={connection.get('period', '')}>
              { period_options }
            </select>
          </div>
        </div>
  ...
  renderConnectionButton = (key, connection) => {
    const { openReceivers } = this.state;
    console.log('or', openReceivers);

    if (openReceivers[0] !== undefined) {
console.log('or_0', openReceivers[0]);
    }
    return (
      // (<Button onClick={this.openReceiver(connection, key)} bsStyle="link">
      //   Receiver
      // </Button>)
      <div>
        {(openReceivers[key] !== undefined)
        ? (<Button onClick={this.closeReceiver(connection, key)} bsStyle="link">
  ...
  render() {
    const { settings,
            ...
            keyValue,
            keyLabel} = this.props;
    const { receiverType, showRemoveKey, openReceivers } = this.state;
    ...
    return (
      <div className="ReceiverSettings">
        {
          settings.map((connection, key) => this.renderConnectionButton(key, connection))
        }
        { openReceivers.map(connection => this.renderConnection(connection)) }
      </div>
    );
  }
}
